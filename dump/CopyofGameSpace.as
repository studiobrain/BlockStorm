package physics
{
	...
	public class GameSpace extends Sprite
	{
		...
		{
			//GameSpace.space.listeners.add(new InteractionListener(CbEvent.BEGIN, InteractionType.ANY, BOXED, CLEAR, checkForMatch));
			//GameSpace.space.listeners.add(new InteractionListener(CbEvent.END, InteractionType.ANY, BOXED, CLEAR, checkedMatch));
			GameSpace.space.listeners.add(new PreListener(InteractionType.COLLISION, ONE_WAY, BOXED, oneWayHandler));
		}
		
		private function checkForMatch(cb:InteractionCallback):void
		{
			var boxShape:Body = cb.int1.castBody;
			var armShape:Body = cb.int2.castBody;
			
			//blockList = armShape.interactingBodies(InteractionType.SENSOR, 1);
			
			/*var potential:BodyList = getRemovalBlocks();
			
			if (potential.length > 2)
			
			potential.foreach(function(removal:Body):void 
			{
				removal.userData.graphic.parent.lightenUp(true);
			});*/
		}
		
		/*private function checkedMatch(cb:InteractionCallback):void
		{
			var boxShape:Body = cb.int1.castBody;
			
			if (boxShape.userData.graphic.parent) 
			{
				boxShape.userData.graphic.parent.lightenUp(false);
			}
		}*/
		...
		/*public function getRemovalBlocks():void
		{
			...
			removalBlocks.foreach(function(removal:Body):void 
			{
				removal.userData.graphic.parent.lightenUp(true);
			});
		...
		{
			trace("blocksToDestroy: " + blocksToDestroy);
			//var blocksToDestroy:BodyList = getRemovalBlocks();
		...
		{
			trace("restoreBlocks() removal: " + removal);
			while (!removal.empty()) {
